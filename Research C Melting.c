...
/*
 * Give the value for kv in the adjoint integral equation-> part of the green's function
 * Parameters 
 *    x    radius at the given timestep
 *    xtau the radius spanning up to the given timestep
 *    t    the time at our timestep
...
double kvAdj(double x, double xtau,double t, double tau){
  double g;
...
/*
 * Give the value for kernel in the adjoint integral equation corresponding to dyG
 * Parameters 
 *    x    radius at the given timestep
 *    xtau the radius spanning up to the given timestep
 *    t    the time at our timestep
...
double kdAdj(double x, double xtau,double t, double tau){
  double g;

  g=0.5*M_SQPINV*(exp(-(x-xtau)*(x-xtau)/(4.0*(t-tau)))*(x-xtau)/(2.0*(t-tau))-exp(-(x+xtau)*(x+xtau)/(4.0*(t-tau)))*(-(x+xtau)/(2.0*(t-tau))));
...
double zeroLay(int n,double h,double *r,double *q){
  double tn=n*h;
  int i;
  double sum=0.5*kv(r[n],0.0,tn,0.0)*q[0]/sqrt(tn-0.0);
  
  for (i=1; i<n; i++){
    sum += kv(r[n],0.0,tn,i*h)*q[i]/sqrt(tn-i*h);
  }
  return sum*h;
}
...
/*
 * Algorithm for finding the mu value in the single layer discretization
 * Parameters 
 *    n    timestep index
 *    h    dt
 * Return
...
double munSLay(int n,double h, double *r){
  double tn=n*h, dn, Kn, r2;
  int i;
  double sum;
  
  sum = 0.5*kv(r[n],r[n],tn,0)/sqrt(tn-0.0);
  for (i=1; i<n; i++){
    sum += kv(r[n],r[n],tn,i*h)/sqrt(tn-i*h);
  }

  Kn = sqrt(tn/M_PI)*(exp(-r[n]*r[n]/tn)*(1.0+exp(r[n]*r[n]/tn)))-r[n]*erfc(r[n]/sqrt(tn));
  return Kn - sum*h;
}

/*
 * Algorithm for finding the mu value in the zero layer discretization
 * Parameters 
 *    n    timestep index
 *    h    dt
 * Return
...
double munZLay(int n,double h, double *r){
  double tn=n*h, dn, Kn, r2;
  int i;
  double sum;
  
  sum = 0.5*kv(0,r[n],tn,0)/sqrt(tn-0.0);
  for (i=1; i<n; i++){
    sum += kv(0,r[n],tn,i*h)/sqrt(tn-i*h);
  }

  Kn = 2.0*exp(-r[n]*r[n]/(4.0*tn))*sqrt(tn/M_PI)-r[n]*erfc(r[n]/(2.0*sqrt(tn)));
  return Kn - sum*h;
}
...
double qtjAdj(int n, double h, double *r, double *qt){
  double ktntjsum=0.0 , tn=n*h;
  int i;
  
  //edited to contain q not qtilde
  for (i=1;i<n;i++){
    ktntjsum+=kvAdj(r[n],r[i],tn,i*h)*qt[i]/(sqrt(tn-i*h));
  }
...
double qtoAdj(int n, double h, double *r){
  double ktntjsum=0.0 , tn=n*h;
  int i;
  
  for (i=1;i<n;i++){
    ktntjsum+=kvAdj(r[n],r[0],tn,i*h)*sqrt(tn-i*h)/(tn*sqrt(i*h));
  }
  
  //return 1/(tn*sqrt(4.0*M_PI))*(r[n]*exp(-r[n]*r[n]/(4.0*tn))*sqrt(M_PI*tn)+M_PI*(tn-r[n]*r[n]/2.0)*erfc(r[n]/(2.0*sqrt(tn))))-h*ktntjsum;
  return 0-h*ktntjsum;
}

/*
 * Algorithm for finding the coefficient of q[n] in the single layer of the adjoint
 * Parameters 
 *    n    timestep index
 *    h    dt
 * Return
...
double qtnAdj(int n, double h, double *r){
  double sum=0.0,tn=(double)n*h,Kn;
  int i;

  for (i=1;i<n;i++){
    sum+=kvAdj(r[n],r[n],tn,i*h)*sqrt(i*h)/(sqrt(tn)*sqrt(tn-i*h));
  }

  Kn=sqrt(M_PI)/4.0-1/(tn*sqrt(4.0*M_PI))*(-r[n]*exp(-r[n]*r[n]/tn)*sqrt(M_PI*tn)+0.5*M_PI*(2.0*r[n]*r[n]+tn)*erfc(r[n]/sqrt(tn)));
...
/*
 * Algorithm for finding the double layer potential in the adjoint integral equation
 * Parameters 
 *    n    timestep index
 *    h    dt
 *    r    radius time history
...
double dLayAdj(int n,double h,double *r,double *q){
  double tn=n*h;
  int i;
  double sum=0.5*kdAdj(r[n],r[0],tn,0.0)*q[0]/sqrt(tn-0.0);
  
  for (i=1; i<n; i++){
    sum += kdAdj(r[n],r[i],tn,i*h)*q[i]/sqrt(tn-i*h);
  }
...
/*
 * Algorithm for finding the mu value in the discretization of the double layer potential in the adjoint integral equation
 * Parameters 
 *    n    timestep index
 *    h    dt
 * Return
...
double munAdj(int n,double h,double *r){
  double tn=n*h,Kn;
  int i;
  double sum=0.5*kdAdj(r[n],r[n],tn,0)/sqrt(tn-0.0);
  
  for (i=1; i<n; i++){
    sum += kdAdj(r[n],r[n],tn,i*h)/sqrt(tn-i*h);
  }
  
  Kn=0.5*erfc(r[n]/sqrt(tn));
...
void solveState(int n, double tmax, double *r,double *qs,double *dr){
  ...
  for (j=1;j<=n;j++){
    qs[j]=-1.0*(sLay(j,dt,r,qs)+zeroLay(j,dt,r,fs)+munZLay(j,dt,r)*fs[j])/(munSLay(j,dt,r));
  }
...
void solveAdj(int n, double tmax, double *r ,double *qs,double *qarev){
  int h,i , j,k,l,m;
  double *qsrev, *rrev, *qa;
  double dt,totmax,totmaxinv,xk,tk;
  
  qa=(double*)malloc( (n+1)*sizeof(double) );
  qsrev=(double*)malloc( (n+1)*sizeof(double) );
  ...
  for (j=1;j<n;j++){
    qa[j]=(-qsrev[0]/2.0+dLayAdj(j,dt,rrev,qsrev)+munAdj(j,dt,rrev)-qtoAdj(j,dt,rrev)*qa[0]-qtjAdj(j,dt,rrev,qa))/qtnAdj(j,dt,rrev);
    if (j==n){
      //printf("%f\n",qtnAdj(j,dt,rrev));
    }
  }
...
void compute_gradient(int N, int M, double tMax, double *qS, double *r,double *g, double *dr){
  ...
  for (j=0;j<=M;j++){
    ...
    for (i=0;i<=N;i++){
      hatr = sqrt(tj)*y[i];
      if (j==0||j==1){
        dhatr = totmax*sqrt(tj)*z[i] + 0;
      }
      else{
        dhatr = totmax*sqrt(tj)*z[i] + 0.5*y[i]/sqrt(tj);
      }
      if (j==0){
        g[i] += 0.5*(qS[j]*dhatr - (qa[j]+qS[j]*dr[j])*hatr*(qS[j]));
      }
      else if(j==M){
        g[i] += 0.5*(qS[j]*dhatr - qS[j]*dr[j]*hatr*(qS[j]));
      }
      else{
        g[i] += qS[j]*dhatr - (qa[j]+qS[j]*dr[j])*hatr*(qS[j]);
      }
    }
  }
  ...
  for (m=0;m<N+1;m++){
    hatr=sqrt(tj)*y[m];
    g[m]=g[m]*dt-mun(M,dt)*qa[M]*(qS[M])*hatr;
  }
...
void test_gradient(int N, int M, double tMax) {
  ...
  compute_gradient(N, M, tMax, q, r,g,dr);
...
void test_optimization(int numInts, int deg, int tmax, double h_0, int inner_itermax,  int line_search_itermax,double init_guess){
  ...
  if (file == NULL) {  
    ...
    for (j=0; j<inner_itermax; j++) {  
      ...
      d_bdry_curve(deg, numInts, a, dr, tmax);
      solveState(numInts,tmax,r,q,dr);
      compute_gradient(deg, numInts, tmax, q, r,g,dr);
...
int main(int nargs, char *argv[]){
  ...
  int i,j,k,x,l,numInts=400,verbose=0,deg=10,M,inner_itermax=100,outer_itermax=10,line_search_itermax=20,job;
  double *r,*y,*z,*a,*q,*qa,*g,*t,*dr,*fs,*qt;
  ...
  qt = (double*)calloc( numInts+1,sizeof(double) );
  g = (double*)calloc( deg+1,sizeof(double) );
  a = (double*)calloc( deg+1,sizeof(double) );
  y = (double*)calloc( deg+1,sizeof(double) );
  z = (double*)calloc( deg+1,sizeof(double) );
...
d_bdry_curve(deg, numInts, a, dr, tmax);
solveState(numInts,tmax,r,q,dr);
solveAdj(numInts,tmax,r,q,qa);

#if 0
for (j=0;j<=numInts;j++){
  printf("r[%d]->%f",j,r[j]);
  printf("dr[%d]->%f",j,dr[j]);
  printf("q[%d]->%f",j,q[j]);
  printf("qa[%d]->%f\n",j,qa[j]);
}
#endif
...
for (l=0;l<=numInts;l++){
  ...
  //printf("%f\n",q[l]);
  //printf("%f\n",sLay(l,dt,r,q));
  //printf("%f\n",sLay(l,dt,r,q)+mun(l,dt)*kvtntn()*q[l]);
  //printf("%f\n",zeroLay(l,dt,r,fs)+0*munZLay(l,dt,r)*fs[l]);
  //printf("%f\n",q[l]);
  printf("%f\n",qa[l]);
}
#endif
